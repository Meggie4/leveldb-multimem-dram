...
//////////////meggie
void VersionSet::PrintCompactionRange(Compaction* c) {
    DEBUG_T("compaction range:\n");
    for(int which = 0; which < 2; which++) {
        DEBUG_T("inputs_[%d]:\n", which);
        for(int i = 0; i < c->inputs_[which].size(); i++) {
           DEBUG_T("smallest:%s, largest:%s\n",
                   c->inputs_[which][i]->smallest.user_key().ToString().c_str(),
                   c->inputs_[which][i]->largest.user_key().ToString().c_str());
        }
    }
}
//////////////meggie
