...
class ThreadPool {
    ...
    void Task() {//线程的执行体
        while( !bailout ) {
            DEBUG_T("%s, to handle job\n", threadpool_name.c_str());
            next_job()();//next_job()函数就是获取想要运行的函数对象，然后执行()表示开始运行
            --jobs_left;//剩下的还未完成的任务数量 
            DEBUG_T("%s, after handle job, jobs_left:%d\n", 
                    threadpool_name.c_str(),
    ...
    std::function<void()> next_job() {
        ...
        if( !bailout ) {
            DEBUG_T("%s, have get job from queue\n", threadpool_name.c_str());
...
public:
    ThreadPool(int threadcount, std::string name)
        : jobs_left( 0 )
        , bailout( false )
        , finished( false ) 
        , threadcount(threadcount)
        , threadpool_name(name)
    {//线程池的构造函数
